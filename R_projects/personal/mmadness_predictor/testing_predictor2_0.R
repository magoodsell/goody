...
curve(exp(beta_1 + beta_2*x)/(1+exp(beta_1 + beta_2*x)), add = TRUE) #, from = 0, to = 130)
...
teams <- list('Rutgers_ScarletKnights', 'NotreDame_FightingIrish', 'Wyoming_Cowboys', 'Indiana_Hoosiers'
              , 'WrightState_Raiders',	'Bryant_Bulldogs','TexasSouthern_Tigers',	'TexasA&MCC_Islanders'
              , 'Gonzaga_Bulldogs',	'GeorgiaState_Panthers','BoiseState_Broncos',	'Memphis_Tigers'
              ,'Connecticut_Huskies',	'NewMexicoState_Aggies','Arkansas_Razorbacks',	'Vermont_Catamounts'
              , 'Alabama_CrimsonTide',	'TexasTech_RedRaiders',	'MontanaState_Bobcats',	'MichiganState_Spartans'
...
teams <- str_flatten(teams, collapse = '|')
team_1_test <- 'Rutgers_ScarletKnights'
team_1_play <- str_match(teams, team_1_test) 
team_1_play <- str_split(team_1_play, '_') 
team_1_play <- team_1_play[[1]]
team_1_play


View(teams_dat)
teams_dat <- teams_dat %>% 
  filter(teams == team_1_test) %>% 
  separate(teams, into = c('school', 'mascot'), sep = '_')
team_1 <- teams_dat$school[1]  
teams_dat[1]
...
teams_dat <- data.frame(teams = c('Rutgers_ScarletKnights', 'NotreDame_FightingIrish', 'Wyoming_Cowboys', 'Indiana_Hoosiers'
  ...
  teams_dat['school'][teams_dat['school'] == "Jackson State"] <- "J'Ville St"
